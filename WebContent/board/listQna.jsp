			...
			<tr>
				<td colspan="6">
					<c:choose>
						<c:when test="${start > 1 }">
							<button type="button"
								onclick="location.href='listQna.jsp?start=${start-1}'">이전</button>
						</c:when>
						<c:otherwise>
					...
					<c:forEach begin="1" end="${pc.totEndPage }" step="1" var="cnt">
						<a href="listQna.jsp?start=${cnt }">[${cnt }]</a>
					</c:forEach> <c:choose>
						<c:when test="${start < pc.totEndPage }">
							<button type="button"
								onclick="location.href='listQna.jsp?start=${start+1}'">다음</button>
						</c:when>
						<c:otherwise>
							<button type="button" disabled>다음</button>
						</c:otherwise>
					</c:choose> ${start } / ${pc.totEndPage }
					<button type="button" onclick="location.href='regFormQna.jsp'">
						글쓰기</button></td>
			</tr>
